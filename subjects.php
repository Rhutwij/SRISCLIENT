...
else
{
    ...
    if(isset($_POST) && !empty($_POST) && isset($_GET['form']) && $_GET['form']=='add')
    {
        ...
        if($error==0)
        {
            $add_response=addSubject($_POST['nameadd'],$userId);
            if($add_response[0]["msg"]=="Subject added")
            {
                $error=0;
            }
            else
    ...
    if(isset($_POST) && !empty($_POST) && isset($_GET['form']) && is_numeric($_GET['form']))
    {
        ...
        {
            //update request
            $edit_reponse=editSubject($_POST['name'.$_GET['form']],$userId,$_GET['form']);
        ...
        {
            //delete request
            $delete_reponse=deleteSubject($userId,$_GET['form']);
  ...
  <body>
    ...
    <br/>
      ...
      <div class="starter-template">
       <p> Add /Remove /Edit Subjects table</p>
      </div>
       <div>
           <table width="100%">
            <thead>
...
<?php
          ...
          {
            echo '<tr id='."tr".$subject['SubjectId'].'><td><form method="POST" action="subjects.php?form='.$subject['SubjectId'].'"><table class="table"><tr><td><input type="text" class="form-control"  name='."name".$subject['SubjectId'].' id='."name".$subject['SubjectId'].' value="'.$subject['Name'].'"></td>
            <td><button type="submit" name='."edit".$subject['SubjectId'].' class="btn btn-success">Edit</button> <button type="submit" name='."delete".$subject['SubjectId'].' class="btn btn-danger">Delete</button></td></td></tr></table></form></tr>';
          }
           echo '<tr id="tradd"><td><form method="POST" action="subjects.php?form=add"><table class="table"><tr><td><input type="text" class="form-control"  name="nameadd" value=""></td>
