...
else
{
    ...
    if(1==$_SESSION['registered'])
    {
        ...
        $professorInfo=getuserInfoById($professorId);
        $roleId=isset($_SESSION) && isset($_SESSION['roleId'])?$_SESSION['roleId']:0;
        $buckets=getBucketsByProfessorId($professorId);
        if($collegeId==0)
        {
            $collegeId=$buckets[0]['CollegeId'];
        }
...
<html lang="en">
  ...
  <body>
    ...
    <br/>
    <br/>
      <h1 class="glyphicon glyphicon-hdd"> Professsor <span class="glyphicon glyphicon-user"> <?php echo $professorInfo[0]['Username'];?> </span> repository</h1>
      <div class="starter-template">
       <p>Buckets of professor</p>
       <ol class="breadcrumb">
        <li><a href="dash.php">Home</a></li>
       ...
       <div>
            <ol>
                ...
                if(!empty($buckets))
                {
                    ...
                    {
                        echo "<li><span class='glyphicon glyphicon-folder-open' aria-hidden='true'></span>   <a href='documents.php?professorId=".$bucket['UserId']."&bucketId=".$bucket['BucketId'].
                        "&subjectId=".$bucket['SubjectId']."&collegeId=".$collegeId."'>"
                        .$bucket['Name']."</a></li>";
                    }
                }
                ?>
            ...
            if(empty($buckets))
            {
                echo "<div class=\"panel panel-warning\"><div class=\"panel-heading\"><h2 class=\"panel-title\">No results</h2></div> No buckets. </div>";
            }
            ?>
       </div>
